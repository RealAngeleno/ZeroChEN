...
sub SetMenuList
{
	...
	$Base->SetMenu('Pool Thread List', "'bbs.pool','DISP','LIST'");
	$Base->SetMenu('<hr>', '');
	$Base->SetMenu('Return to System Management', "'sys.bbs','DISP','LIST'");
}
...
sub PrintThreadList
{
	...
	$Page->Print("Display No<input type=text name=DISPNUM size=4 value=$dispNum>");
	$Page->Print("<input type=button value=\"　Display　\" onclick=\"$common\"></td></tr>\n");
	$Page->Print("<tr><td colspan=5><hr></td></tr>\n");
	$Page->Print("<tr><th style=\"width:30px\"><a href=\"javascript:toggleAll('THREADS')\">All</a></th>");
