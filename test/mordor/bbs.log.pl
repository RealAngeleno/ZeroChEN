...
sub DoPrint
{
	...
	if ($subMode eq 'INFO') {														# トップ画面
		...
		$BASE->PrintComplete('Log operation processing', $this->{'LOG'});
	}
...
sub SetMenuList
{
	...
	$Base->SetMenu('Log Information', "'bbs.log','DISP','INFO'");
	$Base->SetMenu('<hr>', '');
	
	# ログ閲覧権限のみ
	if ($pSys->{'SECINFO'}->IsAuthority($pSys->{'USER'}, $ZP::AUTH_LOGVIEW, $bbs)) {
	$Base->SetMenu('Thread Creation Log', "'bbs.log','DISP','THREADLOG'");
	$Base->SetMenu('HostLog', "'bbs.log','DISP','HOSTLOG'");
	$Base->SetMenu('ErrorLog', "'bbs.log','DISP','ERRORLOG'");
	$Base->SetMenu('<hr>', '');
	}
...
sub FunctionLogDelete
{
	...
	# 既存ログを退避する
	$Logger->MoveToOld();
	push @$pLog, 'Existing logs have been saved...';
	
	# ログのクリアと保存
	$Logger->Clear();
	$Logger->Write();
	$Logger->Close();
	push @$pLog, 'Log deletion complete...';
