...
sub DoPrint
{
	...
	if ($subMode eq 'LIST') {														# グループ一覧画面
		...
		$BASE->PrintComplete('User group processing', $this->{'LOG'});
	}
...
sub SetMenuList
{
	...
	if ($pSys->{'SECINFO'}->IsAuthority($pSys->{'USER'}, $ZP::AUTH_USERGROUP, $bbs)) {
		$Base->SetMenu('User registration', "'bbs.user','DISP','CREATE'");
		$Base->SetMenu('Import users', "'bbs.user','DISP','IMPORT'");
	}
	$Base->SetMenu('<hr>', '');
...
sub PrintGroupSetting
{
	...
	$Page->Print("<center><table border=0 cellspacing=2 width=100%>");
	$Page->Print("<tr><td colspan=2>Enter each information and press the [Set] button.</td></tr>");
	$Page->Print("<tr><td colspan=2><hr></td></tr>");
	$Page->Print("<tr><td class=\"DetailTitle\" colspan=2>Basic information</td></tr>");
	$Page->Print("<tr><td colspan=2><table cellspcing=2>");
	$Page->Print("<tr><td class=\"DetailTitle\">Group name</td><td>");
	...
	$Page->Print("<tr><td class=\"DetailTitle\" width=40%>Permission Information</td>");
	$Page->Print("<td class=\"DetailTitle\">Affiliated users</td></tr><tr><td valign=top>");
	
	# 権限一覧表示
	$Page->Print("<input type=checkbox name=A_USERGROUP $authNum[0] value=on>Management Group Settings<br>");
	$Page->Print("<input type=checkbox name=A_CAPGROUP $authNum[1] value=on>Cap group setting<br>");
	$Page->Print("<input type=checkbox name=A_LOGVIEW $authNum[14] value=on>View/delete log<br>");
	$Page->Print("<hr>");
	$Page->Print("<input type=checkbox name=A_THREADSTOP $authNum[2] value=on>Stop/resume thread<br>");
	$Page->Print("<input type=checkbox name=A_THREADPOOL $authNum[3] value=on>Thread dat dropped/revive<br>");
	$Page->Print("<input type=checkbox name=A_TREADDELETE $authNum[4] value=on>Delete Thread<br>");
	$Page->Print("<input type=checkbox name=A_THREADINFO $authNum[5] value=on>Update thread information<br>");
	$Page->Print("<input type=checkbox name=A_PASTCREATE $authNum[6] value=on>Generate past log<br>");
	$Page->Print("<input type=checkbox name=A_PASTDELETE $authNum[7] value=on>Delete past log<br>");
	$Page->Print("<input type=checkbox name=A_BBSSETTING $authNum[8] value=on>BBS Settings<br>");
	$Page->Print("<input type=checkbox name=A_BBSEDIT $authNum[13] value=on>edit<br>");
	$Page->Print("<input type=checkbox name=A_NGWORDS $authNum[9] value=on>Edit NG Words<br>");
	$Page->Print("<input type=checkbox name=A_ACCESUSER $authNum[10] value=on>Edit access restrictions<br>");
	$Page->Print("<hr>");
	$Page->Print("<input type=checkbox name=A_RESABONE $authNum[11] value=on>RESABONE<br>");
	$Page->Print("<input type=checkbox name=A_RESEDIT $authNum[12] value=on>resedit<br>");
	$Page->Print("</td>\n<td valign=top>");
...
sub PrintGroupDelete
{
	...
	$Page->Print("<center><table border=0 cellspacing=2 width=100%>");
	$Page->Print("<tr><td colspan=2>Remove groups below.</td></tr>");
	$Page->Print("<tr><td colspan=2><hr></td></tr>");
	...
	$Page->Print("※Note: You cannot delete a group you belong to.<br>");
	$Page->Print("※Note: All users who belong to the deleted group will be unaffiliated.</td></tr>");
	$Page->Print("<tr><td colspan=2><hr></td></tr>");
	$Page->Print("<tr><td colspan=2 align=right><input type=button value=\"　Delete　\" ");
...
sub PrintGroupImport
{
	...
	$Page->Print("<tr><td colspan=2><hr></td></tr>");
	$Page->Print("<tr><td class=\"DetailTitle\">Import from another board</td>");
...
sub FunctionGroupSetting
{
	...
	{
		my $id;
	push @$pLog, '■The following groups have been registered. ';
	push @$pLog, "Group name: $name";
	push @$pLog, "Description: $expl";
	push @$pLog, "Permission: $auth";
	push @$pLog, 'user belongs to:';
...
sub FunctionGroupImport
{
	...
	push @$pLog, "Imported group settings for 「$name」.";
